...
variable "enable_custom_domain" {
  ...
  type        = bool
  default     = true  # Production: Always use custom domain
}

variable "assign_public_ip" {
  description = "Assign public IP to ECS tasks (true for dev without NAT, false for staging/prod with NAT)"
  type        = bool
  default     = false  # Production: false (uses NAT Gateway)
}

variable "subdomain" {
  description = "Subdomain for this environment (e.g., 'btg' for prod)"
  type        = string
  default     = "btg"
}
...
variable "sns_admin_emails" {
  description = "List of admin emails to subscribe to SNS topics"
  type        = list(string)
  default     = []
}
